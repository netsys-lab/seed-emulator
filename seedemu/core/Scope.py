...
class Scope:
    ...
    # NOTE ISD scope could be added here, but since ASNs are globally unique they're not strictly required
...
if __name__=="__main__":
    ...
    assert Scope(ScopeTier.Global)> Scope(ScopeTier.AS, as_id=150), 'global scope is superset of AS scopes '
    assert Scope(ScopeTier.AS, as_id=150) < Scope(ScopeTier.Global) , 'AS scopes are subset of global scope '

    cmpr =  Scope.collate 
    assert cmpr(Scope(ScopeTier.AS, as_id=150),
                Scope(ScopeTier.AS, as_id=160))==0, 'disjoint AS scopes'
    assert cmpr(Scope(ScopeTier.Node, as_id=150, node_id='br0'),
                Scope(ScopeTier.Node, as_id=160, node_id='br0'))==0, 'disjoint Nodes scopes different ASes'
    assert cmpr(Scope(ScopeTier.Node, as_id=150, node_id='br0'),
                Scope(ScopeTier.Node, as_id=150, node_id='br1'))==0, 'disjoint Nodes scopes same AS'
    assert cmpr(Scope(ScopeTier.AS, as_id=150,node_type=ScopeType.HNODE),
                Scope(ScopeTier.AS, as_id=150,node_type=ScopeType.BRDNODE))==0, 'disjoint Types scopes at AS level'
    assert cmpr(Scope(ScopeTier.AS, as_id=150,node_type=ScopeType.HNODE),
                Scope(ScopeTier.AS, as_id=160,node_type=ScopeType.BRDNODE))==0, 'disjoint Types as well as ASes'
    assert cmpr(Scope(ScopeTier.Global,node_type=ScopeType.HNODE),
                Scope(ScopeTier.Global,node_type=ScopeType.BRDNODE))==0, 'disjoint Types scopes at global level'

    assert Scope(ScopeTier.AS, as_id=150)>Scope(ScopeTier.Node, as_id=150, node_id='brd0', node_type=ScopeType.BRDNODE) , 'node scope is subset of AS scope'
   
    assert not Scope(ScopeTier.AS, as_id=150)< Scope(ScopeTier.Node, as_id=150, node_id='brd0', node_type=ScopeType.BRDNODE)


    assert Scope(ScopeTier.AS, as_id=160)==Scope(ScopeTier.AS, as_id=160) , 'identical AS scope'
    assert Scope(ScopeTier.AS, as_id=160, node_type=ScopeType.ANY)==Scope(ScopeTier.AS, as_id=160) , 'identical AS scope'
    assert Scope(ScopeTier.AS, as_id=160, node_type=ScopeType.ANY)>Scope(ScopeTier.AS, as_id=160, node_type=ScopeType.BRDNODE) , 'ANY includes all types'
    assert Scope(ScopeTier.AS, as_id=150)!=Scope(ScopeTier.AS, as_id=160) , 'not identical scope'
    
    assert (Scope(ScopeTier.Node, as_id=150,node_id='brd0') ==
            Scope(ScopeTier.Node, as_id=150,node_id='brd0',node_type=ScopeType.BRDNODE)), 'same node but with extra type info'

    assert Scope(ScopeTier.Global) > Scope(ScopeTier.Node, as_id=150, node_id='brd0')
    assert not Scope(ScopeTier.Global, ScopeType.HNODE) > Scope(ScopeTier.Node, as_id=150, node_id='brd0', node_type=ScopeType.BRDNODE) , 'node unaffected by global type'
