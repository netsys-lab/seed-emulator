...
class ScionRouting(Routing):
    ...
    def getAvailableOptions(self):
        return [
            ScionRouting.Option.custom(key, ScionRouting.Option.defaultValue(key))
            for key in valid_keys
        ]

    def __init__(
        self,
        loopback_range: str = "10.0.0.0/16",
    ...
    def _resolveFlag(flag: str, node: Node = None) -> str:
        ...
        if (
            node.getOption("use_envsubst").value == "true"
            and node.getOption(flag).mode == OptionMode.RUN_TIME
        ):
    ...
    def configure(self, emulator: Emulator):
        ...
        for (scope, type, name), obj in reg.getAll().items():
            ...
            if type not in ["hnode", "csnode", "brdnode"]:
                continue
            ...
            if type == "brdnode":
                ...
                name = csnode.getName()
                ctrl_log = (
                    ">> /var/log/scion-control-service.log 2>&1"
                    if nologrotate
                    else " 2>&1 | rotatelogs -n 2 /var/log/scion-control-service.log 1M "
                )
                cs_tmpl = "cs_no_disp" + ("_envsubst" if useenvsubst else "")
                csnode.appendStartCommand(
                    _CommandTemplates[cs_tmpl].format(
                        name=name, cmd=ScionRouting._nameOfCmd("control"), log=ctrl_log
                    ),
                    fork=True,
                )
    ...
    def __provision_dispatcher_config(node: Node, isd: int, as_: ScionAutonomousSystem):
        ...
        if ip is None:
            raise ValueError(
                f"Node {node.getName()} has no interface in the control service network"
            )

        dispatcher_conf = _Templates["dispatcher"].format(isd_as=isd_as, ip=ip)
        if node.getOption("serve_metrics").value == "true":
            dispatcher_conf += _Templates["metrics"].format(
                node.getLocalIPAddress(), 30441
            )
    ...
    def __provision_router_config(router: ScionRouter):
        ...
        name = router.getName()
        config_content = _Templates["general"].format(
            name=name, loglevel=ScionRouting._resolveFlag("loglevel", router)
        )

        _keyvals_router = [
            "bfd.disable={}".format(ScionRouting._resolveFlag("disable_bfd", router))
        ]
        _kvals_features = [
        ...
        if (
            router.getOption("serve_metrics").value == "true"
            and (local_ip := router.getLocalIPAddress()) != None
        ):
    ...
    def _get_networks_from_router(
        ...
        try:
            ...
            raise Exception(
                f"No common network between {router1} and {router2} but they are in the same AS"
            )
            ...
            for other_if in scion_ifs:
                if other_if != interface:
                    ...
                    if br_str == this_br_name:
                        ...
                        net = ScionRouting._get_networks_from_router(
                            this_br_name, br_str, as_
                        )  # get network between the two routers (Assume any two routers in AS are connected through a network)
                        (latency, bandwidth, packetDrop) = (
                            net.getDefaultLinkProperties()
                        )
                        mtu = net.getMtu()
                        ifs["Latency"][str(other_if)] = f"{latency}ms"
                        if bandwidth != 0:  # if bandwidth is not 0, add it
                            ifs["Bandwidth"][str(other_if)] = int(
                                bandwidth / 1000
                            )  # convert bps to kbps
                        ...
                        ifs["Hops"][
                            str(other_if)
                        ] = 1  # NOTE: if interface is on different router, hops is 1 since we assume all routers are connected through a network
    ...
    def _get_xc_link_properties(
        ...
        if_addr = this_br.getScionInterface(interface)["underlay"]["public"].split(":")[
            0
        ]
    ...
    def _get_ix_link_properties(
        ...
        if_addr = IPv4Address(
            this_br.getScionInterface(interface)["underlay"]["public"].split(":")[0]
        )

        # get a list of all ix networks this Border Router is attached to
        ixs = [
            ifa.getNet()
            for ifa in this_br.getInterfaces()
            if ifa.getNet().getType() == NetworkType.InternetExchange
        ]
    ...
    def _provision_staticInfo_config(node: Node, as_: ScionAutonomousSystem):
        ...
        for interface in range(1, as_._ScionAutonomousSystem__next_ifid):
            ...
            if lat != 0:  # if latency is not 0, add it
                if not staticInfo[
                    "Latency"
                ]:  # if no latencies have been added yet empty dict
            ...
            for _if in ifs["Latency"]:  # add intra latency
                if ifs["Latency"][_if] != "0ms":  # omit 0ms latency
                    if not staticInfo["Latency"][str(interface)][
                        "Intra"
                    ]:  # if no intra latencies have been added yet empty dict
                        staticInfo["Latency"][str(interface)]["Intra"] = {}
                    staticInfo["Latency"][str(interface)]["Intra"][str(_if)] = ifs[
                        "Latency"
                    ][_if]
            ...
            staticInfo["LinkType"][
                str(interface)
            ] = "direct"  # NOTE: for now all ASes are connected through CrossConnects which are docker Nets under the hood and thus direct
    ...
    class Option(BaseOption):
        ...
        def supportedModes(self) -> OptionMode:
            return __supported_modes[self._name_]

        @staticmethod
        def defaultValue(key: str) -> str:
            match key.upper():
                case "ROTATE_LOGS":
                    return str(ScionRouting._rotate_logs).lower()
                case "APPROPRIATE_DIGEST":
                    return str(ScionRouting._appropriate_digest).lower()
                case "DISABLE_BFD":
                    return str(ScionRouting._disable_bfd).lower()
                case "EXPERIMENTAL_SCMP":
                    return str(ScionRouting._experimental_scmp).lower()
                case "LOGLEVEL":
                    return ScionRouting._loglevel
                case "SERVE_METRICS":
                    return str(ScionRouting._serve_metrics).lower()
                case "USE_ENVSUBST":
                    return str(ScionRouting._use_envsubst).lower()
                case _:
