...
class Scion(Layer, Graphable):
    ...
    def peekNextIfId( ia: IA) -> int:
        """get the next free IFID, but don't allocate it yet
        subsequent calls return the same, if not interleaved with getNextIfId() or _setIfId()
        """
        ifs = set()
        if ia in Scion.__if_ids_by_as:
            ifs = Scion.__if_ids_by_as[ia]
...
# i.e. a_IF_ID and b_IF_ID if known (i.e. by a DataProvider)
    def addIxLink(self, ix: int, a: Union[IA, Tuple[int, int]], b: Union[IA, Tuple[int, int]],
        ...
        if 'if_ids' in kwargs:
            ids = kwargs['if_ids']
            assert not Scion._setIfId(a, ids[0]), 'interface ID not unique'
            assert not Scion._setIfId(b, ids[1]), 'interface ID not unique'           
