...
client1_ip = f"10.{topo['client1_asn']}.0.71"
client2_ip = f"10.{topo['client2_asn']}.0.71"

try:
    with open('/topo/paths1.json', 'r') as f:
        paths1_data = json.load(f)
except FileNotFoundError:
    paths1_data = {}
try:
    with open('/topo/paths2.json', 'r') as f:
        paths2_data = json.load(f)
except FileNotFoundError:
    paths2_data = {}
...
def get_paths_detailed():
    global paths1_data, paths2_data, links
    for path_id, path in paths1_data.items():
        _links = path['links']
        bw = float('inf')
        latency = 0
        jitter = 0.0
        ...
        paths1_data[path_id] = path

    for path_id, path in paths2_data.items():
        _links = path['links']
        bw = float('inf')
        latency = 0
        jitter = 0.0
