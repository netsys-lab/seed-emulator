...
def run(dumpfile = None):
    ...
    create_as(1, 173, issuer=151)# clientX -knows about site2 in its ISD 1
    create_as(1, 174, issuer=151)
    # non-core AS ISD2
    create_as(2, 230, issuer=241)
    create_as(2, 231, issuer=241) # clientY - knows about site3 in its ISD 2
    ...
    # install DevService on all border routers and control services
    dev_targets.extend(brs.values())
    dev_targets.extend(cses.values())


    for node in dev_targets:
        install_dev_svc(emu, node, devsvc, [repo] )
